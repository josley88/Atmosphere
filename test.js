...
let song_queue = ["hello world"]
var weather_data = null

const _getRecommendations = async (token, genres, seed_tracks, target_valence='', target_energy='', target_tempo=100, market='', limit=30) => {
    ...
    if (target_energy != ''){
        req_url += '&target_energy=' + target_valence
    }
    if (market != ''){
...
_getUserTopItems = async (token, offset) => {
    ...
    // if we are forcing genre, use less seed tracks from top items in recommendations
    if (localStorage.getItem("forceGenre") == "false") {
        req_url += "&limit=5"
    } else {
        req_url += "&limit=0"
    }
...
function getLongPlaylist(){
    ...
    var seed_tracks = topTracks.slice(0,5)
